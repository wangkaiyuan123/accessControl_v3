...
public class TeaEncodeUtil {
    ...
    private static long k0, k1, k2, k3;
...
//            System.out.printf("%08X\t%08X\t%08X\t%08X\n", i, v0, v0 >> 5, k3);
  ...
  //将16进制字节数组转换为16进制字符串  例：{(byte)0x0A,byte0x0B}-->"0A0B"
	...
	/**
     * 
     * @param mac 将mac地址分为3,3,3,3 最高位0填充,取每个字符的ASCII对应的16进制数
     *   如：D8B 04C DEC 6C1  -->
     *          0x00, 0x44, 0x38, 0x42, 
                0x00, 0x30, 0x34, 0x43, 
                0x00, 0x44, 0x45, 0x43, 
                0x00, 0x36, 0x43, 0x31
     * @param time  0x14,0x12,0x01,0x03,0x0B,0x36,0x00,0x03
     * @return 
     */
    public static String teaEncode(byte[] mac,byte[] time){
    	setKey(mac);
    	byte[] enc =  encrypt_group(time);
    	String str = BinaryToHexString(enc);
